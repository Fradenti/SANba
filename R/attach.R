...
SANba_attach <- function() {
  # Create `to_load` which is a character vector of all SANba
  ...
  # SANba with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("SANba ", package_version("SANba"))
  )

  # Return a character string containing the package version for each of SANba's constituents
...
SANba_detach <- function() {
  pak <- paste0("package:", c(pkgs, "SANba"))
  lapply(pak[pak %in% search()], detach, character.only = TRUE)
  invisible()
}

#' List all packages imported by SANba
#'
#' @export
#'
#' @examples
#' SANba_packages()
SANba_packages <- function() {
  # get all imports from SANba's package description file
  raw <- utils::packageDescription("SANba")$Imports
